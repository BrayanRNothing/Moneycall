...
async function main() {
    try {
        ...
        const rows = await db.prepare('SELECT id, nombres, "apellidoPaterno", "apellidoMaterno", telefono, "vendedorAsignado", "equipo_id", "propietarioId" FROM clientes WHERE telefono LIKE ? OR telefono2 LIKE ?')
