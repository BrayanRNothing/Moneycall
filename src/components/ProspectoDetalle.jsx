                                    ...
                                    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
                                        ...
                                        <div id="detalle-prospecto-contacto-rapido" className="ml-auto flex items-center gap-2 shrink-0">
                                            ...
                                            <button
                                                ...
                                                onClick={() => {
                                                    if (tieneWhatsApp) {
                                                        navigate('/vendedor/chats', { state: { clienteId: pid } });
                                                    }
                                                }}
                                                className={`h-8 w-8 inline-flex items-center justify-center rounded-md transition-colors shadow-xs border ${tieneWhatsApp ? 'bg-green-50 hover:bg-green-100 border-green-100 cursor-pointer' : 'bg-slate-100 border-slate-200 cursor-not-allowed opacity-60'}`}
                                                title={tieneWhatsApp ? 'Abrir chat de WhatsApp en el CRM' : 'No hay teléfono para WhatsApp'}
                                            >
                            ...
                            <div className="flex bg-slate-200/60 p-0.5 rounded-xl">
                                ...
                                <button
                                    onClick={() => {
                                        ...
                                        axios.get(`${API_URL}/api/whatsapp/status`, { headers: getAuthHeaders() })
                                            .then(res => {
                                                if (res.data.status === 'desconectado') {
                                                    toast.error('⚠️ Tu sesión de WhatsApp está desconectada. Conéctala en Ajustes para enviar nuevos mensajes.', { duration: 4000 });
                                                }
                                            })
                                            .catch(() => {});
                                    }}
