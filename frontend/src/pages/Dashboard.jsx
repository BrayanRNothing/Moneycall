...
import { getDashboardMetrics, getDailyMeeting, getAlertasS1, getClientes, createAuditoria, getClientesTMUpgrade } from '../api'
...
export default function Dashboard() {
  ...
  const loadActivity = async (range = activityRange) => {
    setActivityLoading(true)
    try {
      const daily = await getDailyMeeting(user.id)
      const tipos = daily.reduce((acc, row) => {
        const source = row.ayer || {}
        acc.S1 = (acc.S1 || 0) + (source.s1 || 0)
        acc.S2 = (acc.S2 || 0) + (source.s2 || 0)
        acc.DC = (acc.DC || 0) + (source.dc || 0)
        acc.PT = (acc.PT || 0) + (source.pt || 0)
        return acc
      }, {})
      setActivity({
        range,
        label: range === 'day' ? 'Hoy' : 'Todos los días',
        llamadasHoy: daily.reduce((sum, row) => sum + (row.ayer?.total || 0), 0),
        salientes: daily.reduce((sum, row) => sum + (row.ayer?.salientes || 0), 0),
        entrantes: daily.reduce((sum, row) => sum + (row.ayer?.entrantes || 0), 0),
        tipos,
        cotizaciones: { sinF1: daily.reduce((sum, row) => sum + (row.alertas?.length ? 1 : 0), 0) }
      })
    } catch {
      setActivity({
        range,
  ...
  useEffect(() => { loadCore() }, [])
  useEffect(() => { loadActivity(activityRange) }, [activityRange])
  ...
  const activityTypes = activity?.tipos || {}
  const quadrants = [
    { id: 'S1', label: 'Cuadrante 1 (S1) — Recuperación', value: `${activityTypes.S1 || 0} llamadas`, percent: Math.min((activityTypes.S1 || 0) * 10, 100), color: '#3b82f6' },
    { id: 'S2', label: 'Cuadrante 2 (S2) — Venta Cruzada', value: `${activityTypes.S2 || 0} llamadas`, percent: Math.min((activityTypes.S2 || 0) * 10, 100), color: '#6366f1' },
    { id: 'DC', label: 'Delivery Check (DC) — Calidad', value: `${activityTypes.DC || 0} llamadas`, percent: Math.min((activityTypes.DC || 0) * 10, 100), color: '#10b981' },
    { id: 'PT', label: 'Prospección Proactiva (PT)', value: `${activityTypes.PT || 0} llamadas`, percent: Math.min((activityTypes.PT || 0) * 10, 100), color: '#ec4899' },
  ]
  ...
  return (
    <div className="space-y-5 min-h-full flex flex-col pb-6">
      <div className="flex items-center justify-between shrink-0">
        ...
        <button onClick={reloadDashboard} className="neu-btn w-8 h-8 rounded-xl flex items-center justify-center" style={{ color: 'var(--text-muted)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-stretch">
        {/* Llamadas del equipo por cuadrante */}
        <div className="neu-card p-4 lg:col-span-2 flex flex-col justify-between">
          <div className="flex items-center justify-between shrink-0 mb-2">
            <h3 className="text-xs font-bold" style={{ color: 'var(--text)' }}>Actividad del Equipo por Tipo de Llamada</h3>
            <div className="flex items-center gap-2">
              <select
                value={activityRange}
                onChange={e => setActivityRange(e.target.value)}
                className="text-[9px] font-bold px-2.5 py-1 rounded-lg outline-none"
                style={{ color: 'var(--text-muted)', background: 'var(--bg)', boxShadow: '2px 2px 4px var(--shadow-dark), -2px -2px 4px var(--shadow-light)' }}
              >
          ...
          {activityLoading ? (
            <div className="space-y-3 flex-1 flex flex-col justify-around py-1 animate-pulse">
              {[1, 2, 3, 4].map(n => (
                <div key={n} className="space-y-1">
                  <div className="h-3 bg-gray-400/10 rounded w-2/3" />
                  <div className="h-1.5 bg-gray-400/10 rounded w-full" />
                </div>
              ))}
            </div>
          ) : (
          ...
          {activity?.cotizaciones?.sinF1 > 0 && (
            <div className="mt-3 p-2.5 rounded-xl flex items-center gap-2" style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
              ...
              <span className="text-[10px] font-semibold" style={{ color: '#f59e0b' }}>
                {activity.cotizaciones.sinF1} cotización(es) sin seguimiento F1 ({activity.label.toLowerCase()})
              </span>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 gap-2.5">
            ...
            <div className="flex gap-2 p-2.5 rounded-xl" style={{ background: 'rgba(16,185,129,0.05)', border: '1px solid rgba(16,185,129,0.2)' }}>
              ...
              <div className="space-y-0.5 min-w-0 flex-1">
                ...
                <p className="text-[10px] font-bold truncate" style={{ color: 'var(--text)' }}>{activityTypes.DC || 0} entregas {activity?.label?.toLowerCase() || 'del periodo'} sin verificar.</p>
